...
using CoherentStructures, Test,Tensors
using Arpack
...
@testset "1d grid piecewise_quadratic on linear" begin
    ...
    u = @inferred nodal_interpolation(ctx,u_fun)
    @test evaluate_function_from_dofvals(ctx,u, Vec{1}((0.0,))) == u_fun([0.0])
    @test evaluate_function_from_dofvals(ctx,u, Vec{1}((0.25,))) == u_fun([0.25])
    @test evaluate_function_from_dofvals(ctx,u, Vec{1}((0.5,))) == u_fun([0.5])
    @test evaluate_function_from_dofvals(ctx,u, Vec{1}((0.75,))) == u_fun([0.75])
    @test evaluate_function_from_dofvals(ctx,u, Vec{1}((1.0,))) == u_fun([1.0])
    for i in 1:500
        p = rand()
        @test abs(evaluate_function_from_dofvals(ctx,u, Vec{1}((p,))) - u_fun([p])) < 1e-14
    end
end
...
@testset "1d grid piecewise_quadratic on quadratic" begin
    ...
    for i in 1:500
        ...
        #    continue
        #end
        @test abs(evaluate_function_from_dofvals(ctx,u, Vec{1}((p,))) - quadfun([p])) < 1e-14
    end
end
...
@testset "1d non-adaptive TO" begin
    for grid in (regular1dP1Grid, regular1dP2Grid)
        ...
        ALPHAS, _ = nonAdaptiveTOCollocation(ctx,finv,project_in=false,bdata_domain=bdata)
        ...
        for i in 1:ctx.n
            ...
            @test abs(pushforward_u_dofs[ctx.node_to_dof[i]] -
                                evaluate_function_from_dofvals(ctx,u_initial_dofs, Vec{1}((finv(curnode)[1],)))) == 0.0
        end
    end
end
